import { parseAtomManifest, type Platform } from '../services/atomParser'
import type { Atom, RepoSource } from '../../renderer/types/atom'
import type { RepoProvider } from './types'

const CONCURRENCY = 25
const MAX_RETRIES = 3
const FETCH_TIMEOUT_MS = 15_000

interface TreeItem {
  type: string
  name: string
  path: string
}

function parseProjectPath(url: string): string {
  // Accept bare owner/repo format
  if (!url.includes('://')) {
    return url
  }
  // Strip https://gitlab.com/ prefix, then remove /-/tree/... suffix
  const withoutOrigin = url.replace(/^https?:\/\/gitlab\.com\//, '')
  // Remove trailing /-/tree/... or /-/blob/...
  return withoutOrigin.replace(/\/-\/.*$/, '')
}

export class GitLabProvider implements RepoProvider {
  async fetchCatalog(
    source: RepoSource,
    onBatch?: (atoms: Atom[]) => void,
    onProgress?: (loaded: number, total: number) => void,
    onEnumProgress?: (counted: number) => void,
    knownAtomDirs?: Set<string>
  ): Promise<Atom[]> {
    const projectPath = parseProjectPath(source.url)
    const encodedPath = encodeURIComponent(projectPath)
    const apiBase = `https://gitlab.com/api/v4/projects/${encodedPath}`
    const rawBase = `https://gitlab.com/${projectPath}/-/raw/master`

    const allPaths = await fetchAllManifestPaths(apiBase, onEnumProgress)
    const toFetch = knownAtomDirs
      ? allPaths.filter(p => !knownAtomDirs.has(p.split('/').slice(0, -1).join('/')))
      : allPaths
    const platform: Platform = process.platform === 'darwin' ? 'Mac' : 'Windows'
    let loaded = allPaths.length - toFetch.length
    onProgress?.(loaded, allPaths.length)

    const results: Atom[] = []

    for (let i = 0; i < toFetch.length; i += CONCURRENCY) {
      const batch = toFetch.slice(i, i + CONCURRENCY)
      const settled = await Promise.allSettled(
        batch.map(p => parseManifest(p, platform, rawBase, projectPath, source))
      )
      const batchResults: Atom[] = []
      for (const s of settled) {
        if (s.status === 'fulfilled' && s.value) {
          results.push(s.value)
          batchResults.push(s.value)
        }
      }
      loaded += batch.length
      if (batchResults.length > 0) onBatch?.(batchResults)
      onProgress?.(loaded, allPaths.length)
    }

    return results
  }

  async fetchFile(source: RepoSource, relativePath: string): Promise<Buffer> {
    const projectPath = parseProjectPath(source.url)
    const rawBase = `https://gitlab.com/${projectPath}/-/raw/master`
    const res = await fetchWithRetry(`${rawBase}/${relativePath}`)
    return Buffer.from(await res.arrayBuffer())
  }

  async countAtoms(source: RepoSource): Promise<number> {
    const projectPath = parseProjectPath(source.url)
    const encodedPath = encodeURIComponent(projectPath)
    const apiBase = `https://gitlab.com/api/v4/projects/${encodedPath}`
    const paths = await fetchAllManifestPaths(apiBase)
    return paths.length
  }
}

async function fetchAllManifestPaths(
  apiBase: string,
  onEnumProgress?: (counted: number) => void
): Promise<string[]> {
  const paths: string[] = []
  let page = 1

  while (true) {
    const url = `${apiBase}/repository/tree?path=Atoms&recursive=true&per_page=100&page=${page}`
    const res = await fetchWithRetry(url)
    const items: TreeItem[] = await res.json()

    for (const item of items) {
      if (item.type === 'blob' && item.name.endsWith('.atom')) {
        paths.push(item.path)
      }
    }

    onEnumProgress?.(paths.length)

    const nextPage = res.headers.get('x-next-page')
    if (!nextPage) break
    page++
  }

  return paths
}

async function parseManifest(
  manifestPath: string,
  platform: Platform,
  rawBase: string,
  projectPath: string,
  source: RepoSource
): Promise<Atom | null> {
  try {
    const res = await fetchWithRetry(`${rawBase}/${manifestPath}`)
    const content = await res.text()

    const parts = manifestPath.split('/')
    const atomName = parts[parts.length - 2]
    const category = parts[parts.length - 3] ?? 'Uncategorized'
    const atomDir = parts.slice(0, -1).join('/')

    const parsed = parseAtomManifest(content, platform)

    return {
      id: atomName,
      name: parsed.name || atomName,
      category: parsed.category || category,
      author: parsed.author,
      version: parsed.version,
      description: parsed.description,
      deployFiles: parsed.deployFiles,
      thumbnailUrl: `${rawBase}/${atomDir}/img/${atomName}.png`,
      gitlabUrl: `https://gitlab.com/${projectPath}/-/tree/master/${atomDir}`,
      atomDir,
      hasFilesForPlatform: parsed.hasFilesForPlatform,
      sourceId: source.id,
      sourceName: source.name,
    }
  } catch {
    return null
  }
}

async function fetchWithRetry(url: string): Promise<Response> {
  let lastErr: Error | null = null
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      const res = await fetch(url, { signal: AbortSignal.timeout(FETCH_TIMEOUT_MS) })
      if (res.ok) return res
      if ((res.status === 429 || res.status === 503) && attempt < MAX_RETRIES) {
        await sleep(Math.pow(2, attempt) * 500)
        continue
      }
      throw new Error(`HTTP ${res.status}`)
    } catch (e) {
      lastErr = e as Error
      if (attempt < MAX_RETRIES) await sleep(Math.pow(2, attempt) * 500)
    }
  }
  throw lastErr ?? new Error(`Failed to fetch ${url}`)
}

function sleep(ms: number) {
  return new Promise(resolve => setTimeout(resolve, ms))
}
